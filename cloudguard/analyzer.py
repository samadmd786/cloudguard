...
AWS Security Hub finding (JSON) and sends it to the NVIDIA NIM API (Gemma 4)
using a zero-shot prompt. It forces the model to return a structured JSON
response detailing the risk, business impact, and specific remediation steps.
"""
import json
...
# System prompt instructs the model to act as a senior AWS security engineer
...
def _extract_json(raw: str) -> str:
    """Strip accidental markdown fences and thinking blocks, then return clean text."""
    text = raw.strip()
...
def analyze_finding(finding: dict, api_key: str = None) -> dict:
    """
    Send a Security Hub finding to NVIDIA NIM (Gemma 4) and return a structured analysis.
    ...
    Args:
        ...
        api_key (str, optional): NVIDIA API key. If not provided, it falls
            back to the `GROQ_API_KEY` environment variable.

    Returns:
        dict: The structured analysis parsed from the model's JSON response, or
    ...
    if not key:
        ...
        raise ValueError(
            "GROQ_API_KEY not set. Export it in your shell or pass it directly."
        )
    ...
    try:
        ...
        if "401" in err or "403" in err:
            log.error("Authentication failed — check GROQ_API_KEY")
